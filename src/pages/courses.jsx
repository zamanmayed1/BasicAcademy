    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `http://basicacademy-backend.local/wp-json/wp/v2/course`,
          {
            params: {
              search: searchQuery,
            },
      ...
      <div className="p-2 my-4 rounded-md bg-white max-w-7xl mx-auto py-20 flex justify-center">
        <div className="relative w-[900px] ">
          ...
          {isSearchClicked && (
            <button
              onClick={clearSearch}
              className="ml-2 bg-gray-300 text-gray-700 px-4 py-3 rounded-md focus:outline-none hover:bg-gray-400"
            >
              Clear
            </button>
          )}
